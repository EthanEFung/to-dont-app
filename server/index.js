...
app.get('/todonts', (req, res) => {
  db.collection('todonts').find({}).toArray((err, docs) => {
    if (err) { return console.alert(`could not GET todonts from the database`) }
    res.send(docs);
  })
})
...
app.post('/save', (req, res) => { 
  ...
  db.collection('todonts').insertOne({ text: req.body.text, gotTime: req.body.gotTime })
  res.send('received')
})
...
app.delete('/delete', (req, res) => {
  ...
  db.collection('todonts').deleteOne({ text: req.body.text, gotTime: req.body.gotTime })
    .then(() => {
      db.collection('todonts').find({}).toArray((err, docs) => {
...
MongoClient.connect(url, (err, database) => {
  ...
  todonts.insertMany([{ text: "Use Shitty APIs", gotTime: true },
   { text: "Use APIs that look promising but are actually shit", gotTime: true },
